...
// 原型
    Image.prototype = {
        ...
        _createInsertPanel: function _createInsertPanel() {
             ...
             * zhuzi修改：根据图片地址插入图片
             * @param url 图片地址
             * @param isMultip 是否是多图
             */
            function insertByURL(url, isMultip) {
                if(!url){
                    return;
                }
            ...
            // 开始打开zhuzi的文件管理弹窗
            obj.trigger("EDITOR_IMAGE_UPLOAD", {'src': '', 'isMultip': true});
            obj.one("EDITOR_IMAGE_RETURN", function (event, data) {
                if (typeof data == "string" && data) {//单图；多图
                    insertByURL(data, false)
                }
                if (typeof data == 'object' && data[0]) {
                    insertByURL(data, true)
                }
            });
        ...
        _pasteHandle: function _pasteHandle() {
            ...
            $textElem.on('paste', function (e) {
                ...
                if (pasteTextHandle && typeof pasteTextHandle === 'function') {
                    // zhuzi修改：注释掉。由于只能粘贴纯文本，所以不用获取pasteHtml了
                ...
                // zhuzi修改：所有粘贴，只能粘贴纯文本
                editor.cmd.do('insertHTML', '<p>' + pasteText + '</p>');
                return;

                // // zhuzi修改:注释掉。code 中只能粘贴纯文本
...
// 原型
    UploadImg.prototype = {
        ...
        insertLinkImg: function insertLinkImg(link) {
            ...
            // caiyichen 图片必须被p标签包裹已达到独占一行
            editor.cmd.do('insertHTML', '<p><img src="' + link + '"/></p>');
